...
export function SnapshotDetailView({ snapshot }: Props) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-2">{snapshot.teamName}</h1>
      ...
      {columnData.length > 0 && (
        <div className="space-y-6">
          <h2 className="text-lg font-semibold">カラム詳細</h2>
          {columnData.map((col, i) => (
            <div key={i} className="border rounded-lg p-4">
              <h3 className="font-medium mb-2">{col.name} ({col.cards.length})</h3>
              <ul className="space-y-1">
                {col.cards.map((card, j) => (
                  <li key={j} className="text-sm text-gray-700 flex justify-between">
                    <span>{card.content}</span>
                    {card.votes > 0 && <span className="text-gray-400">{card.votes} 票</span>}
                  </li>
                ))}
              </ul>
            </div>
