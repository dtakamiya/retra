...
export function MemoItem({ memo, cardId }: Props) {
    ...
    return (
      <div className="bg-gray-50 rounded p-2">
        <textarea
          ...
          maxLength={2000}
          className="w-full resize-none border border-gray-200 rounded px-2 py-1 text-xs text-gray-700 focus:outline-none focus:ring-1 focus:ring-indigo-300 min-h-[32px]"
          autoFocus
          rows={2}
        />
  ...
  return (
    <div className="group bg-gray-50 rounded p-2 flex gap-2">
      <div className="flex-1 min-w-0">
        <p className="text-xs text-gray-700 whitespace-pre-wrap break-words">{memo.content}</p>
        <span className="text-[10px] text-gray-400 mt-0.5 block">{memo.authorNickname}</span>
      </div>
      {(canEdit || canDelete) && (
